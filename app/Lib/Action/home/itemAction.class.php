...
class itemAction extends frontendAction
{
    ...
    public function index()
    {
        ...
        $uid = $info['id'];
        // 她的更多搭配
        $more_item = $item_mod->query("select * from {$this->prefix}item where uid=$uid and id !=$id order by add_time desc limit 100");
        shuffle($more_item);
        $more_item = array_slice($more_item, 0, 7);
        $this->assign('more_item', $more_item);
        //她喜欢的搭配
        $she_like_item = $item_mod->query("select * from {$this->prefix}item_like where uid=$uid limit 100");
        shuffle($she_like_item);
        $she_like_item = array_slice($she_like_item, 0, 3);
        $this->assign('she_like_item', $she_like_item);
        //你可能喜欢的
        $rand_like_item = $item_mod->query("select * from {$this->prefix}item where uid!=$uid limit 100");
        shuffle($rand_like_item);
        $may_like_item = array_slice($rand_like_item, 0, 3);
        $this->assign('may_like_item', $may_like_item);
        //前一个，后一个
        $prev_item = $item_mod->query("select * from {$this->prefix}item where uid=$uid and id<$id limit 1");
        $this->assign('prev_item', $prev_item);
        $next_item = $item_mod->query("select * from {$this->prefix}item where uid=$uid and id>$id limit 1");
        ...
        foreach($cmt_list as &$v){
            $v['replay']=$item_comment_mod->where(array('comment_id'=>$v['id']))->order('id DESC')->limit($pager->firstRow . ',' . $pager->listRows)->select();
        }
    ...
    public function comment()
    {
        ...
        if ($comment_id) {
            ...
            $user_mod = D('user');
            if ($user_mod->is_msg_notification($this->visitor->info['id'])) {
                //有人关注我、评论我的搭配或者给我留言
                //解析邮件模板
                $mail_body = D('message_tpl')->get_mail_info('send_comment', $item);
                //发送邮件
                $email = $user_mod->where(array('id' => $item['uid']))->getField('email');
                $email && $this->_mail_queue($email, '您的'.$item['title']."有一个评论", $mail_body);
            }
