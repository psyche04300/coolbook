...
class userAction extends userbaseAction
{
    ...
    public function index()
    {
        if (IS_POST) {
            ...
            //新增的
            $data['profession'] = $this->_post('profession', 'intval');
            $data['blog'] = $this->_post('blog', 'intval');
            $data['weibo'] = $this->_post('weibo', 'intval');
            $data['fashion_statement'] = $this->_post('fashion_statement', 'intval');
    ...
    public function follow()
    {
        ...
        if ($user_mod->is_msg_notification($this->visitor->info['id'])) {
            $user = $user_mod->field('id,email,username')->where(array('id' => $uid))->find();
            //解析邮件模板
            $mail_body = D('message_tpl')->get_mail_info('follow_user', $user);
            $user['email'] && $this->_mail_queue($user['email'], $user['username'] . "关注了你", $mail_body);
        }
